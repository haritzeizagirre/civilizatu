  ...
  template: `
    <div class="auth-backdrop">
      <div class="auth-card">
        ...
        <p class="link">Have an account? <a routerLink="/login">Login</a></p>
      </div>
    </div>
  `,
  styles: [`
    ...
    button { background: #8b6914; color: #fff8e1; border: none; padding: 0.75rem; border-radius: 4px; font-family: 'Cinzel', serif; font-size: 1rem; cursor: pointer; }
...
export class RegisterComponent {
  ...
  submit(): void {
    ...
    this.auth.register({ username: username!, email: email!, password: password! }).subscribe({
      ...
      error: (e: any) => { this.error = e.error?.detail || 'Registration failed'; this.loading = false; },
    });
  }
}
